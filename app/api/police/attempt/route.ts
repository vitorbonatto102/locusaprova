...
export async function POST(request: Request) {
  ...
  try {
    ...
    await db.insert(objectiveAttempts).values({ id: attemptId, userId: USER_ID, examTargetId: session.examTargetId, questionId: question.id, sessionType: session.sessionType, answerJson: JSON.stringify(answer), isCorrect, rawScore, confidence, durationSeconds: Math.max(0, Math.min(3600, Number(payload.durationSeconds) || 0)), errorCategory: category, createdAt: now });
    let earliestReviewMs = Number.POSITIVE_INFINITY;
    for (const nodeId of question.topicIds) {
      const [prior] = await db.select().from(knowledgeMastery).where(and(eq(knowledgeMastery.userId, USER_ID), eq(knowledgeMastery.knowledgeNodeId, nodeId))).limit(1);
      ...
      await db.insert(knowledgeMastery).values({ id: prior?.id ?? crypto.randomUUID(), userId: USER_ID, knowledgeNodeId: nodeId, mastery: next.mastery, stabilityDays: next.stabilityDays, evidenceCount: next.evidenceCount, state: next.state, nextReviewAt: next.nextReviewAt, lastReviewedAt: now, updatedAt: now }).onConflictDoUpdate({ target: [knowledgeMastery.userId, knowledgeMastery.knowledgeNodeId], set: { mastery: next.mastery, stabilityDays: next.stabilityDays, evidenceCount: next.evidenceCount, state: next.state, nextReviewAt: next.nextReviewAt, lastReviewedAt: now, updatedAt: now } });
      await db.insert(knowledgeReviews).values({ id: crypto.randomUUID(), userId: USER_ID, knowledgeNodeId: nodeId, questionId: question.id, reason: isCorrect ? "spaced-repetition" : category ?? "error", scheduledFor: next.nextReviewAt, completedAt: null, createdAt: now });
      if (!isCorrect && nodeId === question.topicIds[0]) await db.insert(learningErrors).values({ id: crypto.randomUUID(), userId: USER_ID, attemptId, questionId: question.id, knowledgeNodeId: nodeId, category: category ?? "knowledge-gap", suggestedCategory: category ?? "knowledge-gap", confidence, suspectedMisconception: question.misconception, nextReviewAt: next.nextReviewAt, resolvedAt: null, createdAt: now, updatedAt: now });
    }
    ...
    await db.update(objectiveSessions).set({ currentIndex: nextIndex, status: complete ? "completed" : "active", updatedAt: now, completedAt: complete ? now : null }).where(and(eq(objectiveSessions.id, session.id), eq(objectiveSessions.userId, USER_ID)));
